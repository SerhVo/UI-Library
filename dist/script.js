...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.carousel = function () {
  Array.from(this).forEach(carousel => {
    ...
    const width = parseInt(window.getComputedStyle(carousel.querySelector(".carousel-inner")).width);
    ...
    (0,_core__WEBPACK_IMPORTED_MODULE_0__["default"])(carousel.querySelector('[data-slide="next"]')).click(e => {
      e.preventDefault();
      moveNext();
    });

    // Добавляем обработчик события на кнопку "prev" для перехода к предыдущему слайду
    (0,_core__WEBPACK_IMPORTED_MODULE_0__["default"])(carousel.querySelector('[data-slide="prev"]')).click(e => {
      e.preventDefault();
      movePrev();
    });
...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.tab = function () {
  for (let i = 0; i < this.length; i++) {
    (0,_core__WEBPACK_IMPORTED_MODULE_0__["default"])(this[i]).on("click", () => {
      (0,_core__WEBPACK_IMPORTED_MODULE_0__["default"])(this[i]).fadeIn(1500).addClass("tab-item--active").siblings().removeClass("tab-item--active").closest(".tab").find(".tab-content").removeClass("tab-content--active").eq((0,_core__WEBPACK_IMPORTED_MODULE_0__["default"])(this[i]).index()).addClass("tab-content--active");
    });
  }
};
...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.fadeToggle = function (dur, display = "block", fin) {
  for (let i = 0; i < this.length; i++) {
    ...
    this._fade(dur, isHidden ? 0 : 1, isHidden ? 1 : 0, isHidden ? display : "none", fin, i);
  }
  return this;
};
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype._fade = function (dur, startOpacity, endOpacity, display, fin, index = null) {
  for (let i = index ?? 0; i < (index !== null ? index + 1 : this.length); i++) {
    this[i].style.display = display;
    const fade = complection => {
      this[i].style.opacity = startOpacity + (endOpacity - startOpacity) * complection;
...
(0,_lib_lib__WEBPACK_IMPORTED_MODULE_0__["default"])().get("https://jsonplaceholder.typicode.com/photos").then(res => console.log(res));
})();
